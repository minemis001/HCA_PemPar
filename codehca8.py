...
N = 1000
TARGET = 5
NDIM = 8  # ganti angka ini saja kalau mau tambah dimensi lagi
...
if rank == 0:
    ...
    data_mentah = np.column_stack((usia, pendapatan, frekuensi,waktu_kunjungan, durasi, jumlah_tiket, 
                                   kepuasan, membership)).astype(np.int32)
else:
    data_mentah = None
...
for i in range(local_n):
    umur, pend, freq, waktu, dur, tiket, kep, memb = local_data[i]
...
#mpiexec -n 10 "C:\Users\ACER\AppData\Local\Python\pythoncore-3.14-64\python.exe" codehca8.py  